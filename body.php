  ...
  <div class="container">
    <div class="row py-5">
      <div class="col">
        ...
        <ul>
          <?php for($a = 0; $a < count($arreglo); $a++){print('<li>'.$arreglo[$a].'</li>');
          }?>
        </ul>
        <hr>
        <pre class="my-3"><code><?php print_r($arreglobjeto);?></code></pre>
        <p>Mi nombre es <i><?php print($arreglobjeto[nombres].' '.$arreglobjeto[apellidos]);?> </i>, tengo <?php print($arreglobjeto[edad]);?> años, mi signo es <?php print($arreglobjeto[signo]);?> y mi animal favorito es el <?php print($arreglobjeto[animal]);?>.
        </p>
        <hr>
        ...
        <p> Tengo información de los planetas de StarWars <strong><i> <?php for($i = 0; $i < count($csv); $i++){
          if($i !=count($csv)-2){
            print($csv[$i][name].', ');
          }else if($i==6){
            print($csv[$i][name].' y ');
